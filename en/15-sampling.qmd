...
### How many points, computed rather than guessed

"As many as you can afford" is the usual answer and it is not an answer. Olofsson's formula gives a defensible one: choose the precision you need on overall accuracy, and it returns the sample size that delivers it.

$$n = \left(\frac{\sum_i W_i S_i}{SE}\right)^2 \qquad S_i = \sqrt{U_i(1 - U_i)}$$

$W_i$ is the proportion of the map that class $i$ covers, $U_i$ is the user's accuracy you expect from it, and $SE$ is the standard error you are willing to report. The classes you expect to be worst contribute most, which is the formula telling you something true about where your effort should go.
...
classes = ["Water", "Other forest", "Mangrove", "Bareland", "Restoration"]
weights = [0.40, 0.25, 0.20, 0.12, 0.03]     # share of the map
expected = [0.90, 0.80, 0.75, 0.70, 0.65]    # user's accuracy you expect

rows = []
for se in [0.02, 0.015, 0.01, 0.005]:
    n = bk.sample_size(se, expected, weights)
    rows.append({
        "Target SE": f"±{se:.1%}",
        "Total points": n,
        "Proportional share for the 3% class": round(n * 0.03),
        "Field days at 25 points a day": round(n / 25),
    })

bk.table(pd.DataFrame(rows), caption=(
    "Precision is bought at a quadratic price. Halving the standard error "
    "quadruples the fieldwork, which is the calculation to do before "
    "promising anybody a number."
))
```

Two things fall out of that table, and both change how a project is planned.

Precision is quadratic. Going from plus or minus two percent to plus or minus one percent takes four times the points, not twice. If a funder asks for a tighter number late in a project, this is the arithmetic that says what it costs.

And proportional allocation starves the class you care about. At a standard error of one percent, the three percent restoration class gets about 42 points if you allocate by area. Forty two points cannot support a user's accuracy claim for that class; the confidence interval around it will be twenty points wide. This is why Olofsson's own recommendation is to set a floor, typically 50 to 100 points per class, and to take the extra points from the classes that are easy.

::: {.callout-important}
## The rare class is the whole point
A restoration project is judged on the restored area, which is the smallest and rarest class on the map. Proportional sampling gives that class the least evidence and the widest error bars, which means the one number the project exists to report is the one the sampling design left unsupported. Set the floor first, then allocate what is left.
:::
...
def sample_size(target_se, expected_accuracies, weights):
    """Olofsson's n for a target standard error of overall accuracy."""
    s = [math.sqrt(u * (1 - u)) for u in expected_accuracies]
    return math.ceil((sum(w * si for w, si in zip(weights, s)) / target_se) ** 2)

classes  = ["Water", "Other forest", "Mangrove", "Bareland", "Restoration"]
weights  = [0.40, 0.25, 0.20, 0.12, 0.03]
expected = [0.90, 0.80, 0.75, 0.70, 0.65]

n = sample_size(0.01, expected, weights)
print(f"total points for SE ±1%: {n}\n")

floor = 75      # minimum points per class, whatever its area
print(f"{'class':14s} {'area':>6s} {'proportional':>13s} {'with a floor':>13s}")
for name, w in zip(classes, weights):
    print(f"{name:14s} {w:5.0%} {round(n * w):13d} {max(round(n * w), floor):13d}")

# Try this: set the expected accuracy of Restoration to 0.5, the worst a
# class can honestly be, and run again. A class you expect to get wrong
# drives the total sample size, which is the formula arguing for pilot work
